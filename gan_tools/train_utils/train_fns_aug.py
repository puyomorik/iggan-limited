...
def GAN_training_function(G, D, GD1, GD2, z_, y_, ema, state_dict, config):
    ...
    def train(x, y):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                # real_samples = 0.8 * x[counter] + 0.2 * rand_translation(x[counter])
                # real_fake_samples = jigsaw_k(x[counter], k = 2)
                # real_fake_samples = stitch(x[counter], k = 2)
                ...
                D1_fake, D1_real, D1_real_fake = GD1(z_[:config['batch_size']], y_[:config['batch_size']], x[counter],
                                                     stitch(x[counter], k=2),
                                                     y_counter, train_G=False, split_D=config['split_D'])
                D2_fake, D2_real = GD2(z_[:config['batch_size']], y_[:config['batch_size']], x[counter],
                                       y_counter, train_G=False, split_D=config['split_D'])
                # y_.sample_()
                ...
                D1_loss_real, D1_loss_fake, D1_loss_real_fake = discriminator_loss(
                    D1_fake, D1_real, D1_real_fake)
                D2_loss_real, D2_loss_fake = losses.discriminator_loss(
                    D2_fake, D2_real)
                if config['curriculum_baseline'] and state_dict['epoch'] <= config['curriculum_epochs']:
                    D_loss = (D1_loss_real + D1_loss_fake) / float(config['num_D_accumulations'])
                else:
                    D_loss = (D1_loss_real + config['alpha'] * D1_loss_fake + config[
                        'beta'] * D1_loss_real_fake + D2_loss_real + D2_loss_fake) / \
        ...
        for accumulation_index in range(config['num_G_accumulations']):
            ...
            if not config['conditional']:
                y_.zero_()
            D_fake = GD1(z_, y_, train_G=True, split_D=config['split_D']) + GD2(z_, y_, train_G=True,
                                                                                split_D=config['split_D'])
            # DD_fake = GDD(z_, y_, train_G=True, split_D=config['split_D'])
        ...
        out = {'G_loss': float(G_loss.item()),
               'D_loss_real': float(D1_loss_real.item() + D2_loss_real.item()),
               'D_loss_fake': float(D1_loss_fake.item() + D2_loss_fake.item())}
...
def GAN_log_function(G, D, GD, z_, y_, ema, state_dict, config):
    ...
    def train():
        ...
        corrupt_dict = {'elastic': 'elastic_transform', 'jpeg': 'jpeg_compression', 'speckle': 'speckle_noise',
                        'gaussian': 'gaussian_noise',
                        'blur': 'gaussian_blur',
                        'zoom': 'zoom_blur', 'brightness': 'brightness', 'contrast': 'contrast',
                        'defocus': 'defocus_blur',
                        'fog': 'fog', 'frost': 'frost', 'glass': 'glass_blur', 'impulse': 'impulse_noise',
                        'motion': 'motion_blur',
                        'pixelate': 'pixelate', 'saturate': 'saturate', 'shot': 'shot_noise', 'snow': 'snow',
                        'spatter': 'spatter',
        ...
        for mode in corrupt_dict:
            ...
            print("logp calculation done for the corruption ", mode, " and the shape of data is ", logp.shape,
                  " min is ", np.min(logp),
                  " max is ", np.max(logp))
...
def update_FID(G, D, G_ema, state_dict, config, FID, experiment_name, test_log, epoch):
    ...
    state_dict['best_FID'] = min(state_dict['best_FID'], FID)
    # Log results to file
    test_log.log(epoch=epoch, itr=int(state_dict['itr']), IS_mean=float(0),
